...
median = np.median(np.array(importances))

# step 5 Predict data
Y_pred = decision_tree_regressor.predict(X_test)

# step 6 Select features that have an importance greater than a threshold (e.g., median)
important_indices = [index for index, importance in enumerate(importances) if importance > median]
...
Y_pred_re = tree_model_selected.predict(X_test_selected)
...
for i, item in enumerate(Y_test):
    print(100 * (1 - abs(item - Y_pred_re[i]) / item))

accuracy = 100 * (1 - abs((Y_test - Y_pred_re) / Y_test))
print(accuracy.mean())
